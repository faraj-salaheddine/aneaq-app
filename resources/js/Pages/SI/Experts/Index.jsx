...
export default function Index({ experts = [] }) {
    ...
    const resetAll = () => { setFilters({ nom: "", prenom: "", email: "", telephone: "", type_etablissement: "" }); setSearch(""); };

    const extraCols = activeCols.filter(c => c.key !== "nom");
    const gridCols  = `260px ${extraCols.map(() => "1fr").join(" ")} 170px`;
                ...
                <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 18, boxShadow: "0 2px 12px rgba(0,0,0,0.06)", overflow: "hidden" }}>
                    ...
                    <div style={{ padding: "1.25rem 1.5rem", borderBottom: "1px solid #f1f5f9", display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap", background: "#fafbfc" }}>
                        ...
                        <div style={{ position: "relative", minWidth: 220, flex: 1 }}>
                            ...
                            <input className="filter-input" type="text" placeholder="Recherche globale..." value={search} onChange={e => setSearch(e.target.value)}
    ...
    <div className="col-menu" style={{ position: "fixed", right: 24, top: 160, background: "#fff", border: "1px solid #e2e8f0", borderRadius: 14, boxShadow: "0 12px 36px rgba(0,0,0,0.12)", zIndex: 999, width: 240, maxHeight: "60vh", overflowY: "auto" }}>
                    ...
                    <div style={{ minWidth: `${260 + activeCols.filter(c => c.key !== "nom").length * 160 + 170}px` }}>
                    ...
                    {filtered.map((expert, i) => {
                        ...
                        return (
                            <div key={expert.id} className="row-hover"
                                style={{ display: "grid", gridTemplateColumns: gridCols, padding: "13px 24px", borderBottom: i < filtered.length - 1 ? "1px solid #f8fafc" : "none", alignItems: "center", transition: "background 0.1s" }}
                            >
                                {activeCols.map(col => {
                                    if (col.key === "nom") return (
                                        <div key="nom" style={{ display: "flex", alignItems: "center", gap: 12 }}>
                                            ...
                                            <div style={{ minWidth: 0 }}>
                                                ...
                                                {expert.grade && (
                                                    <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 2, fontWeight: 500 }}>{expert.grade}</div>
                                                )}
                                            </div>
                                        </div>
                                    );
                                    ...
                                    if (col.key === "telephone") return (
                                        <span key="telephone" style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 12, color: "#64748b", fontFamily: "'DM Mono', monospace" }}>
                                            <svg width={11} height={11} viewBox="0 0 24 24" fill="none" stroke="#94a3b8" strokeWidth="2">
                                                <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.61 3.4 2 2 0 0 1 3.6 1.22h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.8a16 16 0 0 0 6.29 6.29l.95-.96a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/>
                                            </svg>
                                            {expert.telephone || "—"}
                                        </span>
                                    );
                                    ...
                                    return (
                                        <span key={col.key} style={{ fontSize: 12, color: "#64748b" }}>{expert[col.key] || "—"}</span>
                                    );
                                })}
                    ...
                    {filtered.length === 0 && (
                        <div style={{ padding: "5rem 2rem", textAlign: "center" }}>
                            ...
                            <p style={{ fontSize: 16, fontWeight: 700, color: "#374151", margin: "0 0 6px", letterSpacing: "-0.01em" }}>Aucun expert trouvé</p>
                            <p style={{ fontSize: 13, color: "#9ca3af", margin: "0 0 1.5rem" }}>
                                {search || activeFiltersCount > 0 ? "Essayez d'autres critères de recherche" : "Commencez par ajouter un expert à la base de données"}
                            </p>
                            {(search || activeFiltersCount > 0) && (
                                <button onClick={resetAll}
                                    style={{ padding: "9px 22px", borderRadius: 9, border: "none", background: GREEN, color: "#fff", fontSize: 13, fontWeight: 600, cursor: "pointer", boxShadow: `0 4px 12px rgba(29,158,117,0.3)` }}
                                >
                    ...
                    {filtered.length > 0 && (
                        <div style={{ padding: "12px 24px", borderTop: "1px solid #f1f5f9", background: "#fafbfc", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                            <span style={{ fontSize: 12, color: "#94a3b8", fontWeight: 500 }}>
                                Affichage de <strong style={{ color: "#374151" }}>{filtered.length}</strong> sur <strong style={{ color: "#374151" }}>{experts.length}</strong> experts
                            </span>
                            <span style={{ fontSize: 11, color: "#94a3b8", display: "flex", alignItems: "center", gap: 5, fontWeight: 500 }}>
                                <svg width={11} height={11} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                    <rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/>
                                    <rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/>
                                </svg>
                                {activeCols.length} colonnes actives
                            </span>
                        </div>
                    )}
                </div>
