...
export default function ExpertCalendrier({ visites }) {
    ...
    return (
        <ExpertLayout>
            ...
            <div style={{ maxWidth: 960, margin: '0 auto', padding: '24px 16px', display: 'flex', gap: 24, flexWrap: 'wrap' }}>
                <div style={{ flex: '1 1 560px', minWidth: 300 }}>
                    <div style={{ marginBottom: 20 }}>
                        <h1 style={{ fontSize: 20, fontWeight: 700, color: BLUE, margin: 0 }}>Mon calendrier de visites</h1>
                    ...
                    <div style={{ background: '#fff', borderRadius: 18, border: '1px solid #e2e8f0', overflow: 'hidden', boxShadow: '0 4px 16px rgba(0,0,0,0.06)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px', background: `linear-gradient(135deg, ${GREEN}, #178a63)`, color: '#fff' }}>
                                ...
                                return (
                                    <div
                                        ...
                                        style={{
                                            ...
                                            background: isSelected ? `${GREEN}15` : isToday ? '#fef9c3' : 'transparent',
                                            border: isSelected ? `1.5px solid ${GREEN}` : isToday ? '1.5px solid #fbbf24' : '1.5px solid transparent',
                                            transition: 'all 0.15s',
                                        }}
                                    >
                                        <span style={{
                                            display: 'block', textAlign: 'center', fontSize: 13, fontWeight: isToday || isSelected ? 800 : 500,
                                            color: isToday ? '#92400e' : isSelected ? GREEN : '#334155',
                                            marginBottom: 3,
                                        }}>
                                            {day}
                                        </span>
                    ...
                    {selected && selectedVisites.length > 0 && (
                        <div style={{ marginTop: 16, background: '#fff', borderRadius: 14, border: `1.5px solid ${GREEN}40`, padding: '14px 18px', boxShadow: '0 4px 16px rgba(29,158,117,0.1)' }}>
                            <div style={{ fontWeight: 700, fontSize: 14, color: GREEN, marginBottom: 12 }}>
                                {selected} {MOIS[month]} {year}
                            </div>
                                ...
                                return (
                                    <div key={v.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 0', borderBottom: '1px solid #f1f5f9' }}>
                                        <div style={{ flex: 1 }}>
                                            ...
                                            <div style={{ fontSize: 11, color: '#94a3b8' }}>{v.reference} · {v.role_expert}</div>
                                        </div>
                                        ...
                                        <Link href={`/expert/dossiers/${v.id}`} style={{ fontSize: 11, color: GREEN, fontWeight: 600, textDecoration: 'none' }}>
                                            Voir →
                                        </Link>
                                    </div>
                                );
                ...
                <div style={{ flex: '0 1 260px', minWidth: 220 }}>
                    <div style={{ marginBottom: 12, fontWeight: 700, fontSize: 14, color: GREEN }}>Prochaines visites</div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                            {upcomingVisites.map(v => {
                                ...
                                return (
                                    <Link
                                        key={v.id}
                                        href={`/expert/dossiers/${v.id}`}
                                        style={{ textDecoration: 'none', display: 'block', padding: '12px 14px', background: '#fff', borderRadius: 12, border: '1px solid #e2e8f0', boxShadow: '0 2px 6px rgba(0,0,0,0.04)' }}
                                    >
