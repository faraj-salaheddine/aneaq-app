...
export default function RecommandationsDomaines({
    ...
    const filtered = useMemo(() => recommandations.filter(r => {
        ...
        return true;
    }), [recommandations, filterU, filterD]);
        ...
        <>
            ...
            <style>{`
                ...
                *, *::before, *::after { box-sizing: border-box; font-family: 'DM Sans', system-ui, sans-serif; }
                ...
                @keyframes slideDown{ from { opacity:0; transform:translateY(-10px); max-height:0; } to { opacity:1; transform:translateY(0); max-height:600px; } }
                ...
                .urgence-swatch { transition: transform .12s, box-shadow .12s, border-color .12s; cursor: pointer; }
            ...
            <div style={{ minHeight: '100vh', background: 'linear-gradient(160deg,#f0f4f8 0%,#e8eef5 100%)', padding: '2rem 1.5rem 5rem' }}>
                <div style={{ maxWidth: 1180, margin: '0 auto' }}>
                        ...
                        <div style={{ background: `linear-gradient(135deg, ${BLUE} 0%, #1558a8 55%, #0d6abf 100%)`, padding: '2rem 2.5rem', position: 'relative', overflow: 'hidden' }}>
                            ...
                            <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 20, flexWrap: 'wrap' }}>
                                <div>
                                    ...
                                    <h1 style={{ margin: 0, fontSize: 29, fontWeight: 900, color: '#fff', letterSpacing: '-0.5px' }}>Recommandations par domaine</h1>
                    ...
                    {/* ── TABLE CARD ── */}
                    <div className="fu d2" style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 20, overflow: 'hidden', boxShadow: '0 4px 24px rgba(12,68,124,.08)' }}>
                        ...
                        <div style={{ padding: '1rem 1.5rem', borderBottom: '1px solid #f1f5f9', background: '#fafbfc', display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                            ...
                            <div style={{ marginLeft: 'auto' }}>
                                <button className="add-btn" onClick={() => { setShowAddPanel(p => !p); setEditId(null); }}
                                    style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '10px 20px', borderRadius: 11, background: showAddPanel ? '#e2e8f0' : BLUE, color: showAddPanel ? '#374151' : '#fff', fontSize: 13, fontWeight: 800, border: 'none', cursor: 'pointer', boxShadow: showAddPanel ? 'none' : `0 4px 14px ${BLUE}33`, letterSpacing: '.01em' }}>
                                    {showAddPanel
                                        ? <><IcoX size={13}/> Fermer</>
                                        ...
                                        {filtered.map((rec, idx) => {
                                            ...
                                            return (
                                                <tr key={rec.id} className="tr-hover"
                                                    style={{ borderBottom: '1px solid #f1f5f9', background: isEditing ? `${BLUE}05` : 'transparent', borderLeft: `4px solid ${u.color}` }}>
                                                    ...
                                                    <td style={{ ...TD, textAlign: 'right' }}>
                                                        <div className="row-actions" style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}>
                                                            <IconBtn title="Modifier" color={BLUE} bg={`${BLUE}0d`} onClick={() => { setEditId(rec.id); setEditForm({ recommandation: rec.recommandation, urgence: rec.urgence }); setShowAddPanel(false); }}>
                                                                <IcoEdit />
                                                            </IconBtn>
                        ...
                        {/* Table footer */}
                        <div style={{ padding: '.75rem 1.5rem', borderTop: '1px solid #f1f5f9', background: '#fafbfc', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
                            <span style={{ fontSize: 12, color: '#94a3b8', fontWeight: 600 }}>
                                {filtered.length} / {counts.total} entrée{counts.total !== 1 ? 's' : ''}
                                {(filterU !== 'all' || filterD !== 'all') && (
                                    <button onClick={() => { setFilterU('all'); setFilterD('all'); }}
