...
export default function Show({ expert, documents = {} }) {
    ...
    const formatSize = s => s < 1024 * 1024 ? `${(s / 1024).toFixed(1)} Ko` : `${(s / 1024 / 1024).toFixed(1)} Mo`;

    const totalDocs = Object.values(documents).reduce((acc, arr) => acc + (Array.isArray(arr) ? arr.length : 0), 0);
    ...
    return (
        <>
            ...
            <div className="exp-root" style={{ padding: "2.5rem 3rem", minHeight: "100vh", background: "linear-gradient(160deg, #f8fafc 0%, #f1f5f9 100%)" }}>
                ...
                {/* Flash message */}
                {flash?.success && (
                    <div style={{ marginBottom: 20, padding: "12px 18px", borderRadius: 12, background: "#ecfdf5", border: "1px solid #6ee7b7", display: "flex", alignItems: "center", gap: 10 }}>
                        <svg width={16} height={16} viewBox="0 0 24 24" fill="none" stroke={GREEN} strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>
                        <span style={{ fontSize: 13, fontWeight: 600, color: "#065f46" }}>{flash.success}</span>
                    </div>
                )}

                {/* Breadcrumb */}
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 20 }}>
                    <button onClick={() => router.visit("/expert/dashboard")}
                    ...
                    <span style={{ fontSize: 12, color: GREEN, fontWeight: 600 }}>Mon profil</span>
                </div>
                ...
                <div className="fade-up" style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 20, padding: "2rem", boxShadow: "0 2px 12px rgba(0,0,0,0.06)", marginBottom: 24, position: "relative", overflow: "hidden" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "flex-start", gap: 20, position: "relative" }}>
                        ...
                        <div style={{ width: 72, height: 72, borderRadius: 18, background: av.bg, color: av.color, fontWeight: 800, fontSize: 24, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, border: `2px solid ${av.color}20`, letterSpacing: "0.02em" }}>
                            {expert ? initials(expert.nom, expert.prenom) : "?"}
                        </div>
                        ...
                        <div style={{ flex: 1 }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                                <h1 style={{ fontSize: 22, fontWeight: 700, color: "#0f172a", margin: 0, letterSpacing: "-0.02em" }}>
                                    {expert ? <>{expert.prenom} <span style={{ fontWeight: 500, color: "#64748b" }}>{expert.nom}</span></> : "Profil incomplet"}
                                </h1>
                        ...
                        {/* Edit button */}
                        <button
                            onClick={() => router.visit("/expert/profil/modifier")}
                            style={{ display: "flex", alignItems: "center", gap: 7, padding: "10px 18px", borderRadius: 10, border: `1px solid ${GREEN}20`, background: `${GREEN}08`, color: GREEN, fontSize: 13, fontWeight: 600, cursor: "pointer", transition: "all 0.15s", flexShrink: 0 }}
                ...
                {!expert ? (
                    <div style={{ textAlign: "center", padding: "3rem", background: "#fff", borderRadius: 18, border: "1px solid #e2e8f0" }}>
                        <div style={{ width: 56, height: 56, borderRadius: 16, background: `${GREEN}12`, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}>
                            <svg width={24} height={24} viewBox="0 0 24 24" fill="none" stroke={GREEN} strokeWidth="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                        </div>
                        <p style={{ fontSize: 15, fontWeight: 700, color: "#0f172a", margin: "0 0 8px" }}>Profil non configuré</p>
                        <p style={{ fontSize: 13, color: "#94a3b8", margin: "0 0 20px" }}>Complétez votre profil pour accéder à toutes les fonctionnalités.</p>
                        <button onClick={() => router.visit("/expert/profil/modifier")}
                            style={{ padding: "10px 24px", borderRadius: 10, border: "none", background: GREEN, color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer" }}
                        >
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                            ...
                            <div className="fade-up" style={{ animationDelay: "0.05s" }}>
                                ...
                                >
                                    ...
                                    <InfoRow label="Établ. & année resp." value={expert.etablissement_et_annee_responsabilite} />
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                            ...
                            <div className="fade-up" style={{ animationDelay: "0.15s" }}>
                                <Section title="Mes documents" subtitle={`${totalDocs} document${totalDocs !== 1 ? "s" : ""} disponible${totalDocs !== 1 ? "s" : ""}`} color={ORANGE}
                                                ...
                                                return (Array.isArray(docs) ? docs : []).map(doc => (
                                                    ...
                                                    >
                                                        ...
                                                        <div style={{ flex: 1, minWidth: 0 }}>
                                                            <p style={{ fontSize: 13, fontWeight: 700, color: "#0f172a", margin: 0 }}>{meta.label}</p>
                                                        ...
                                                        <a
                                                            href={`/expert/documents/${doc.id}/telecharger`}
                                                            download={doc.original_name}
                                                            style={{ display: "flex", alignItems: "center", gap: 5, padding: "6px 11px", borderRadius: 8, border: `1px solid ${meta.color}25`, background: `${meta.color}10`, cursor: "pointer", fontSize: 11, fontWeight: 600, color: meta.color, textDecoration: "none", transition: "all 0.15s", flexShrink: 0 }}
...
Show.layout = page => <ExpertLayout>{page}</ExpertLayout>;
